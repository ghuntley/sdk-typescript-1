...
export class ThreadedVMWorkflowCreator implements WorkflowCreator {
  ...
  async createWorkflow(
    ...
    const workflow = await VMWorkflowThreadProxy.create(
      this.workerThreadClients[workflowThreadIdx],
      info,
      interceptorModules,
      randomnessSeed,
      now
...
export class VMWorkflowThreadProxy implements Workflow {
  static async create(
    ...
    await workerThreadClient.send({ type: 'create-workflow', info, interceptorModules, randomnessSeed, now });
    return new this(workerThreadClient, info.runId);
  }
